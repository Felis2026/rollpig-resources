...
本文用于指导部署自己的小猪资源作为本地 Overlay 叠加到公有基础包之上。

## 适合什么场景

- 只想给自己的 Bot 增加小猪，不想提交到公开资源仓库。
- 想在本机或服务器目录里维护小猪，不额外部署静态资源服务。
- 想同时使用公有基础包、GIF小猪 Overlay、PJSK 主题 Overlay 和自己的私有包。
...
- `pig.json` 只写你新增的小猪；新增 ID 不能和公有基础包、前序私有包重复。
...
新增小猪时不需要这个文件。只有想覆盖公有基础包已有猪的文案或图片字段时才使用：
...
RollPig Plus `0.8.2+` 会在内部固定叠加 GIF小猪 Overlay，用户自建本地包只需要填写自己的 Overlay。
...
```text
公有基础包 -> 第 1 个私有包 -> 第 2 个私有包 -> 第 3 个私有包
```

约束：

- 只有 manifest 声明 `allow_override: true` 时，后加载的私有包才可以通过 `pig_overrides.json` 覆盖前面已有的猪。
- 覆盖对象中省略的字段会继续使用前一层的值；`null` 不表示删除，当前协议不支持通过覆盖包删除小猪或字段。
...
### GIF小猪 Overlay 可以放进本地私有包吗？

可以。`nonebot-plugin-rollpig-plus >= 0.8.2` 支持 GIF小猪 Overlay 和多个私有 Overlay。
